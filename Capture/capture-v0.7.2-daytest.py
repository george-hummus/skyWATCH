'''
Dat testing version of v0.7.2 of the capture script for the skyWATCH camera - depends on the Cfunctions.py file.
...
templims = [[[122,172],[520,570]],[[107,157],[28,78]]] # inital limits for the template matching
...
while True:
    ...
    while active == True:
        ...
        if active == False:
            ...
            testlog(path,["finshed glancing @ ", dt.datetime.now().strftime("%H:%M:%S"),f"- with exptime of {temp_exptime/1e6}s\n"]) ##

            # check the status of the dome using glance #
            domestatus,templims = dome_detect(glance_path,templates,templims,dynamic=True)
        ...
        # CLOSED DOME MODE #
            ...
            placeholder(devname, prams["location"], timestr, img_path, glance_path)
